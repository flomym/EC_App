...
RSpec.feature "Order items", type: :feature do
	scenario 'A user can order 2 hochyo' do
		...
		visit root_path
		click_on '調理器具' click_on '包丁'
		fill_in '数量', with: '2' click_on 'カートに追加'
		expect(page).to have_content('ご注文ありがとうございました') expect(page).to have_content(%r!注文日時: \d{4}/\d{2}/\d{2} \d{2}:\d{2}!)
